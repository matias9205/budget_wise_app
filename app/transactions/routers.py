...
class TransactionRouter(Security):
    ...
    def _add_routes(self):
        ...
        def fetch_transactions(db_: Session = Depends(db.get_db)):
            return TransactionService(db_).get_all_transactions()
        
        @self.transaction_router.get(f"/{id}", response_model=TransactionSchema)
        def fetch_transaction(id:int, db_: Session = Depends(db.get_db)):
            return TransactionService(db_).get_transaction(id)
        
        @self.transaction_router.post("/new", response_model=TransactionSchema)
        def register_new_transaction(payload: NewTransactionSchema, db_: Session = Depends(db.get_db), token: str = Depends(self.get_oauth2_scheme())):
            ...
            return TransactionService(db_).create_new_transaction(user['username'], payload)